...
class GetUserNodeTool(Tool):
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage]:
        try:
            ...
            try:
                response = client.user.get_node(
                    user_id=tool_parameters["user_id"]
                )
            except NotFoundError:
                raise Exception("User does not exist")

            yield self.create_text_message(response.node.summary)
            yield self.create_json_message({"status": "success", "response": json.loads(response.json())})
